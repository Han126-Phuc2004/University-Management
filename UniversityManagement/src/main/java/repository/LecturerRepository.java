...
public class LecturerRepository {
    ...
    public boolean addLecturer(Lecturer lecturer) {
        ...
        if (existsByEmail(lecturer.getEmail())) {
            return false;
        }
        if (!departmentExists(lecturer.getDepartmentId())) {
            return false;
        }
    ...
    public Lecturer findById(String lecturerId) {
        ...
        try (PreparedStatement stmt = connection.prepareStatement(sql)) {
            ...
            System.err.println("Lỗi khi tìm giảng viên: " + e.getMessage());
        }
        return null;
    }
    ...
    private Lecturer map(ResultSet rs) throws SQLException {
        ...
        if (dob != null) {
            l.setDateOfBirth(dob.toLocalDate());
        }
    ...
    public void saveLecturerFromCSVToBD(String filePath) {
        ...
        System.out.println("Đã lưu thành công " + successCount + "/" + lecturers.size() + " lecturers từ file CSV vào database.");
