...
public class ReadCourseFile {
    ...
    private static Integer mapDepartment(String depCode) {
        if (depCode == null) {
            return 0;
        }
    ...
    public static List<Course> parseCourses(List<String[]> csvData) {
        ...
        for (int i = start; i < csvData.size(); i++) {
            ...
            Integer lecturerId = FileUtils.extractDigitsToInt(safe(cols, 4)); // "GV006" -> 6
            Integer departmentId = mapDepartment(safe(cols, 5));                 // "SE" -> 1
            // semester_id trong CSV là chuỗi kiểu "SEMTEST" -> entity của bạn là Integer, nên để null
    ...
    public static void printCourses(List<Course> courses) {
        ...
        for (Course c : courses) {
            System.out.printf("%s | %s | credits=%d | deptId=%d | lecturerId=%d | max=%d | enrolled=%d%n",
                    c.getCourseId(),
                    c.getCourseName(),
                    c.getCredits(),
                    c.getDepartmentId(),
    ...
    public static void main(String[] args) {
        String filePath = System.getProperty("user.dir") + "/src/main/java/data/courses.csv";
